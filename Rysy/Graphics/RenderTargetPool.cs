...
public static class RenderTargetPool {
    ...
    private static bool _registeredCleanup;
    ...
    public static RenderTargetPoolRef Get(int w, int h) {
        if (!Interlocked.CompareExchange(ref _registeredCleanup, true, false)) {
        ...
        lock (Items) {
            ...
            var target = new RenderTarget2D(Gfx.Batch.GraphicsDevice, w, h, mipMap: false,
                SurfaceFormat.Color, DepthFormat.None, preferredMultiSampleCount: 0, RenderTargetUsage.PreserveContents);
        ...
        lock (Items) {
            ...
            for (int i = items.Count - 1; i >= 0; i--) {
                ...
                if (!item.HasReference && time - item.LastAccessTime > TimeSpan.FromSeconds(2)) {
                    Logger.Write("RenderTargetPool", LogLevel.Info, $"Cleaned up Render Target {i}.");
                    items.RemoveAt(i);
                    item.DisposeBuffer();
                }
            }
