...
public static class SpriteBatchExtensions {
    ...
    {
        public void DrawLine(Vector2 start, float angle, float len, Color color, float thickness = 1) {
            b.Draw(Gfx.Pixel, start, null, color, angle, Vector2.Zero, new Vector2(len, thickness), SpriteEffects.None, 0f);
        }

        public void DrawLine(Vector2 start, Vector2 end, Color color, float thickness = 1, Vector2 offset = default, float magnitudeOffset = 0f) {
        ...
        public void DrawCircle(Vector2 center, float radius, int segments, Color color, float thickness) {
            if (color.A == 0)
                return;
